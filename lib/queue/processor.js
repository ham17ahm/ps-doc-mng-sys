 ...
 *  2. Emit SSE event with extracted data (minus rawText) so the client
 *     can populate the review form.
...
class ExtractionQueue {
  ...
  async _runJob({ documentId, storedPath, mimeType }) {
    try {
      await connectDB();
      ...
      const extractedData = await extractDocument(buffer, mimeType);
      ...
      // Step 5 — notify client; omit rawText to keep the SSE payload small
      const { rawText, ...reviewableData } = extractedData || {};
      this._emit(documentId, { status: 'extracted', extractedData: reviewableData });
